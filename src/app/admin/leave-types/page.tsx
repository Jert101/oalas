...
export default function LeaveTypesPage() {
  ...
  async function onAdd() {
    ...
    setSubmitting(true)
    try {
      const res = await fetch("/api/admin/leave-types", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, description }) })
      if (!res.ok) throw new Error()
      setName(""); setDescription(""); toast.success("Leave type added"); fetchItems()
    } catch { toast.error("Failed to add leave type") } finally { setSubmitting(false) }
  }
        ...
        <div className="p-4 md:p-6 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Add new leave types</CardDescription>
            </CardHeader>
