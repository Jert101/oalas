...
import {
	...
	listCalendarPeriods,
	listArchivedApplicationsForDepartment,
	listArchivedTravelOrdersForDepartment,
} from "@/lib/archive"
...
export default async function DeanApplicationsArchivePage() {
	...
	if (!dean?.department_id) {
		return (
			<div className="p-6">
				<Card>
					<CardHeader>
						<CardTitle>Applications Archive</CardTitle>
					</CardHeader>
					<CardContent>
	...
	return (
		<div className="p-6 space-y-6">
			<h1 className="text-2xl font-semibold">Applications Archive</h1>
			{periods.map(async (p) => {
				const [apps, travels] = await Promise.all([
					listArchivedApplicationsForDepartment(dean.department_id!, p.calendar_period_id),
					listArchivedTravelOrdersForDepartment(dean.department_id!, p.calendar_period_id),
				])
