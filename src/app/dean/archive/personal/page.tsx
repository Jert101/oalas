...
export default function DeanPersonalArchivePage() {
  ...
  const loadArchivedApplications = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/dean/archive/personal?${queryParams}`)
  ...
  const generateCSVExport = (applications: ArchivedApplication[]) => {
    ...
    a.href = url
    a.download = `dean-personal-archived-applications-${new Date().toISOString().split('T')[0]}.csv`
...
<head>
    <meta charset="UTF-8">
    <title>Dean Personal Archived Applications Report</title>
    <style>
        @media print {
            body { margin: 0; }
...
<body>
    <div class="header">
        <h1>Dean Personal Archived Applications Report</h1>
