...
export default function FinanceArchivePage() {
  ...
  const loadCalendarPeriods = async () => {
    try {
      const response = await fetch('/api/finance/archive/calendar-periods')
      if (response.ok) {
  ...
  const loadArchivedApplications = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/finance/archive?${queryParams}`)
  ...
  const generateCSVExport = (applications: ArchivedApplication[]) => {
    ...
    a.href = url
    a.download = `archived-applications-${new Date().toISOString().split('T')[0]}.csv`
...
<head>
    <meta charset="UTF-8">
    <title>Archived Applications Report</title>
    <style>
        @media print {
            body { margin: 0; }
...
<body>
    <div class="header">
        <h1>Archived Applications Report</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold tracking-tight">Finance Archive</h1>
          <p className="text-muted-foreground">
            Historical view of approved and denied applications organized by academic calendar
          </p>
        </div>
