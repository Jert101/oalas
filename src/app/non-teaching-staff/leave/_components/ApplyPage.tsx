...
export default function ApplyPage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      setIsLoading(true)
      try {
        const response = await fetch("/api/non-teaching-staff/profile")
  ...
  const fetchLeaveBalance = async (type: LeaveType) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/non-teaching-staff/leave/balance?type=${type}`)
      const data = await response.json()
      if (!data.success) throw new Error(data.message)
  ...
  const onSubmit = async (values: LeaveApplication | TravelOrder) => {
    ...
    try {
      const endpoint = applicationType === "leave" 
        ? "/api/non-teaching-staff/leave/apply"
        : "/api/non-teaching-staff/travel/apply"
