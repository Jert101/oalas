...
export default function LeaveApplicationPage() {
  ...
  const checkValidation = async () => {
    try {
      const response = await fetch('/api/non-teaching-staff/validation')
      if (response.ok) {
        const data = await response.json()
  ...
  const handleLeaveTypeSelect = async (leaveType: LeaveType) => {
    ...
    try {
      ...
      const [limitsResponse, balanceResponse] = await Promise.all([
        fetch(`/api/non-teaching-staff/leave-limits?leaveTypeId=${leaveType.leave_type_id}`),
        fetch(`/api/non-teaching-staff/leave-balance?leaveTypeId=${leaveType.leave_type_id}`)
      ])
      ...
      console.log(`Leave balance response status: ${balanceResponse.status}`)
      ...
      if (balanceResponse.ok) {
        ...
        console.error('Leave balance API error:', errorText)
        console.error('Leave balance response status:', balanceResponse.status)
        
        // Try to parse as JSON if possible
        try {
          const errorData = JSON.parse(errorText)
          console.error('Leave balance API error (parsed):', errorData)
          ...
          console.error('Leave balance API error (raw text):', errorText)
        }
      }
  ...
  const handleFormSubmit = async (formData: {
    ...
    try {
      ...
      const response = await fetch('/api/non-teaching-staff/leave/apply', {
        method: 'POST',
        headers: {
  ...
  const handleTravelOrderSubmit = async (formData: {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/non-teaching-staff/travel-order', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
