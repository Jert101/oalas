...
export default function CurrentApplicationPage() {
		...
		const load = async () => {
			try {
				const [periodRes, appRes, userRes] = await Promise.all([
					...
					fetch("/api/non-teaching-staff/leave-applications?status=all&page=1&limit=1"),
				...
				const app = appJson.applications?.[0] || null
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between print:hidden">
				...
				<div className="flex gap-2">
					<Button variant="outline" onClick={() => router.push('/teacher/leave')}>
