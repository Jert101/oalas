...
export function LeaveTypeSelection({ onSelect, onBack, isLoading }: LeaveTypeSelectionProps) {
  ...
  const fetchLeaveTypes = async () => {
    try {
      setLoading(true)
      console.log('Fetching office head leave types...')
      const response = await fetch('/api/office-head/leave-types', { cache: 'no-store' })
      console.log('Office Head leave types response status:', response.status)
      
      if (response.ok) {
        const data = await response.json()
        console.log('Office Head leave types data:', data)
        // Display all leave types returned by the API (no filtering)
        setLeaveTypes(Array.isArray(data) ? data : (data.leave_types || []))
      } else {
        const errorData = await response.json()
        console.error('Office Head leave types API error:', errorData)
        toast.error('Failed to load leave types')
      }
    } catch (error) {
      console.error('Error fetching office head leave types:', error)
      toast.error('Failed to load leave types')
    } finally {
      setLoading(false)
    }
