...
export default function OfficeHeadLeaveApplicationPage() {
  ...
  const checkValidation = async () => {
    try {
      const response = await fetch('/api/office-head/validation')
      if (response.ok) {
        const data = await response.json()
  ...
  const handleLeaveTypeSelect = async (leaveType: LeaveType) => {
    ...
    try {
      console.log(`Office Head fetching data for leave type: ${leaveType.name} (ID: ${leaveType.leave_type_id})`)
      
      // Fetch leave limits and balance for the selected leave type
      const [limitsResponse, balanceResponse] = await Promise.all([
        fetch(`/api/office-head/leave-limits?leaveTypeId=${leaveType.leave_type_id}`),
        fetch(`/api/office-head/leave-balance?leaveTypeId=${leaveType.leave_type_id}`)
      ])

      let limitsData = null
      let balanceData = null

      console.log(`Office Head leave limits response status: ${limitsResponse.status}`)
      console.log(`Office Head leave balance response status: ${balanceResponse.status}`)

      if (limitsResponse.ok) {
        limitsData = await limitsResponse.json()
        setLeaveLimits(limitsData.leaveLimit)
        console.log('Office Head leave limits data:', limitsData)
      } else {
        const errorData = await limitsResponse.json()
        console.error('Office Head leave limits API error:', errorData)
      ...
      if (balanceResponse.ok) {
        ...
        console.log('Office Head leave balance data:', balanceData)
      } else {
        const errorText = await balanceResponse.text()
        console.error('Office Head leave balance API error:', errorText)
        
        // Try to parse as JSON if possible
        try {
          const errorData = JSON.parse(errorText)
          console.error('Office Head leave balance API error (parsed):', errorData)
          ...
          console.error('Office Head leave balance API error (raw text):', errorText)
        }
      }
  ...
  const handleFormSubmit = async (formData: {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/office-head/leave/apply', {
        method: 'POST',
        headers: {
  ...
  const handleTravelOrderSubmit = async (formData: {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/office-head/travel/apply', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      case 'choice':
        return (
          <div className="space-y-6">
            {/* Office Head Auto-Approval Notice */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center gap-2">
                <UserCheck className="h-5 w-5 text-blue-600" />
                <h3 className="text-sm font-medium text-blue-800">Office Head Auto-Approval</h3>
              </div>
              <p className="mt-1 text-sm text-blue-700">
                As an Office Head, your applications will be automatically approved upon submission.
              </p>
            </div>
