...
export default function LeaveApplicationPage() {
  ...
  const checkValidation = async () => {
    try {
      const response = await fetch('/api/teacher/validation')
      if (response.ok) {
        const data = await response.json()
  ...
  const handleLeaveTypeSelect = async (leaveType: LeaveType) => {
    ...
    try {
      ...
      const [limitsResponse, balanceResponse] = await Promise.all([
        fetch(`/api/teacher/leave-limits?leaveTypeId=${leaveType.leave_type_id}`),
        fetch(`/api/teacher/leave-balance?leaveTypeId=${leaveType.leave_type_id}`)
      ])
  ...
  const handleTravelOrderSubmit = async (formData: {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/teacher/travel-order', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
